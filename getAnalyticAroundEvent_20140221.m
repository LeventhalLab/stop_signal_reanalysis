function ansig = getAnalyticAroundEvent_20140221( ch, freqIdx, eventName, trialType, twin, varargin )
%
...
hilbert_directory = '\\141.214.45.212/PublicLeventhal1/dan/stop-signal reanalysis/Hilbert transformed LFP 1 Hz bins';
...
for iTr = 1 : numTrials
    ...
    if ~isfield(ch.trials(trIdx(iTr)).timestamps, eventName); continue; end
    event_ts = ch.trials(trIdx(iTr)).timestamps.(eventName);
    ...
    ansig(numValidTrials, :) = fullsig( startSamp : startSamp+samps_per_window-1 );
end