function extractLFPbyChannels_20151204_Nico(channels, varargin)
...
%     [pn, HSDfn, ext, ~] = fileparts(hsdName);
    ...
    HSDdate = HSDfn(7:14);
    HSDtime = HSDfn(16:23);
    new_HSD_fn = [implantID '_' HSDdate '_' HSDtime];
    ...
    for iSubSession = 1 : 26
        sessionDir = fullfile(openDir, [new_HSD_fn(1:14), letterList(iSubSession)]);
    ...
    %         [pn, ~, ~, ~] = fileparts(c{1}.files.lfp.file);
        ...
        hsdName = fullfile(sessionDir, [new_HSD_fn ext]);
