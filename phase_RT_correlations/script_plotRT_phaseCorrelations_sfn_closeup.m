...
yticks = freqLim;%20:20:100;%min(f):20:max(f);
...
PDFname = fullfile(phase_RTcorr_plots_directory, 'phaseRTcorr_across_subjects_cu.pdf');
...
for iROI = 1 : length(ROI_list)
    ...
    for iEOI = 1 : length(EOI_list)
        ...
        imagesc(t, f, toPlot);
        set(gca,'ydir','normal','ylim',freqLim);
