function [mrv_norm, low_freqs, high_freqs] = phase_amp_Canolty_MI(ch, ...
                                                                    eventList, ...
                                                                    eventtWin, ...
                                                                    trialType, ...
                                                                    varargin)
...
%
% OUTPUTS:
%   Canolty_MI - modulation index. m x n x p x q matrix, where m is the number of
...
skip = ceil(numSamps .*rand(numSurrogates*2,1));
...
for iEventType = 1 : numEventTypes
    ...
    for i_f1 = 1 : num_low_freq
        for i_f2 = 1 : num_high_freq
            ...
            toc
            surrogate_mean = mean(surrogate_mrl, 1);
            surrogate_std  = std(surrogate_mrl, 0 , 1);
            mrv_norm(iEventType, i_f1, i_f2, :) = (squeeze(mrv(iEventType, i_f1, i_f2, :)) - surrogate_mean) ./ surrogate_std;
