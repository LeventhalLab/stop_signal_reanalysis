% script_phaseAmp_surrogates_byTrial_mrl_20140324c
...
for i_chDB = 3 : 3%length(chDB_list)
    ...
    for iTrialType = 1 : length(trialTypeList)
        ...
        surrogate_phaseAmp_metadata.eventList       = eventList{iTrialType};
...
%             skip(find(skip<minSkip))=[];
            ...
            for iCh = 1 : numCh
                ...
                phaseAmp_name_mrl = [ch.name '_surrTrial_phase_amp_mrl.mat'];
...
%                                                                    squeeze(sig_amp(i_f2, :, 1:skip(iSurrogate)-1))];
                                ...
                                surrogate_mrl(iSurrogate) = abs(mean(meanTrial));
