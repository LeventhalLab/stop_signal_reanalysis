...
rootNasDir = ['\\' nasIP];
chDB_directory    = fullfile(rootNasDir, 'PublicLeventhal1\dan\stop-signal reanalysis\stop-signal data structures');
hilbert_1Hz_directory = fullfile(rootNasDir, 'PublicLeventhal1\dan\stop-signal reanalysis\Hilbert transformed LFP 1 Hz bins');
hilbert_025Hz_directory = fullfile(rootNasDir, 'RecordingsLeventhal2\stop-sig_reanalysis BU\Hilbert transformed LFP 025 Hz bins');
phaseRThist_directory = fullfile(rootNasDir, 'PublicLeventhal1\dan\stop-signal reanalysis\phase_RT_histogram_analysis');
phaseAmp_directory = fullfile(rootNasDir, 'PublicLeventhal1\dan\stop-signal reanalysis\phaseAmp_windowed');
...
%         surrogate_phaseAmp_metadata.stepSize        = stepSize(iTrialType);
        ...
        if i_chDB==4
            ...
            startSession = numSessions-6;
        end
        for iSession = startSession : 39%numSessions
