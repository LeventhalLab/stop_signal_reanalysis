...
for i_chDB = 1 : length(chDB_list)
    ...
    for iTrialType = 1 : length(trialTypeList)
        ...
        fig_saveName = [implantID '_mean_phaseAmpPlot_' trialTypeList{iTrialType} '.pdf'];
        fig_saveName = fullfile(subject_phaseAmpdir, fig_saveName);
        
        region_mean_saveName = [implantID '_mean_phaseAmp_byRegion_' trialTypeList{iTrialType} '.mat'];
        region_mean_saveName = fullfile(subject_phaseAmpdir, region_mean_saveName);
        if exist(region_mean_saveName, 'file'); continue; end    % comment this line out to make new plots if means already saved
        ...
        for iSession = 1 : numSessions
            ...
            if ~isfield(region_phaseAmp_metadata, 'low_freqs')   % to fix an oversight in early versions of these files
                ...
                save(session_region_mean_name, 'mean_MI', 'region_phaseAmp_metadata');
            end
...
%                 numCh = length(regionChannels);
        ...
        if length(eventtWin) == 1
            region_phaseAmp_metadata.eventtWin(2) = eventtWin(1) + region_phaseAmp_metadata.analysisWin;
            eventtWin = region_phaseAmp_metadata.eventtWin;
            save(session_region_mean_name, 'mean_MI', 'region_phaseAmp_metadata');   % corrects an oversight in the code that wrote out these structures
        end
        tmin = eventtWin(1);
        ...
        for i_allRegions = 1 : num_allRegions
            ...
            if makePlots
                ...
                if rowNum == 1
                    ...
                    page_numSessionList = num2str(num_regionSessions(i_allRegions));
                    numPages = numPages + 1;
                else
                    page_regionList = [page_regionList ', ' allRegionList{i_allRegions}];
                    page_numSessionList = [page_numSessionList ', ' num2str(num_regionSessions(i_allRegions))];
                end
                if rowNum == 0; rowNum = regions_per_page; end
                
                for iEventType = 1 : numEventTypes
                    
                    toPlot = log10(squeeze(mean_regionMI(i_allRegions, iEventType, 1, :, :))');
                ...
                if (rowNum == regions_per_page || numRegionPlots == num_allRegions)
                    ...
                    textStr{1} = sprintf('%s phase-amplitude coupling, twin = [%d, %d] around %s', implantID, eventtWin(1), eventtWin(2), region_phaseAmp_metadata.eventList{iEventType});
        ...
        save(region_mean_saveName, 'mean_regionMI', 'num_regionSessions', 'allRegionList', 'all_region_phaseAmp_metadata');
