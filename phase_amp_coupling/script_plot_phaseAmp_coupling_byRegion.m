...
for i_chDB = 1 : length(chDB_list)
    ...
    for iTrialType = 1 : length(trialTypeList)
        ...
        for iSession = 1 : numSessions
            ...
            if length(eventtWin) == 1
                ...
                eventtWin = phaseAmp_metadata.eventtWin;
                save( phaseAmp_name, 'MI', 'phaseAmp_metadata');   % corrects an oversight in the code that wrote out these structures
            end
            tmin = eventtWin(1);
            ...
            mean_MI = zeros(numRegions, numEventTypes, 1, length(low_freqs), length(high_freqs));
            ...
            for iRegion = 1 : numRegions
                ...
                MI_by_region{iRegion} = zeros(numCh, numEventTypes, numSteps, length(low_freqs), length(high_freqs));
    ...
    %               iCh
                    ...
                    for iEventType = 1 : numEventTypes
                        ...
                        toPlot = log10(squeeze(MI(iEventType, 1, :, :))');
            ...
            if makePlots
                for iRegion = 1 : numRegions
                    ...
                    for iEventType = 1 : numEventTypes
                        toPlot = log10(squeeze(mean_MI(iRegion, iEventType, 1, :, :))');
                        axes(h_axes(rowNum, iEventType));
                        imagesc(low_freqs, high_freqs, toPlot);
                        set(gca,'ydir','normal','clim',colorLim);
                        if rowNum == 1
