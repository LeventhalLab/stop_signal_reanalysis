...
for i_chDB = 4 : 4%length(chDB_list)
    ...
    for iTrialType = 1 : length(trialTypeList)
        ...
        for iPlotSession = 1 : length(sessions_to_plot)
            
            iSession = sessions_to_plot(iPlotSession);
            sprintf('%d of %d sessions, %s', iPlotSession, length(sessions_to_plot), sessionList{iSession})
            
%         for iSession = 9 : 9%numSessions
...
%             centerFreqs = [centerFreqs_025Hz; centerFreqs_1Hz];
                ...
                for iPlotType = 1 : length(plotTypes)
                    ...
                    for iFreq = 1 : length(plotFreqs{iPlotType})
                        ...
                        fig_saveName{iVar, iPlotType}{iFreq} = ...
                            ['phaseAmp_' trialTypeList{iTrialType} '_' var_to_plot{iVar} '_' plotTypes{iPlotType} '_' freqStr '_' sessionList{iSession} '.pdf'];
                        fig_saveName{iVar, iPlotType}{iFreq} = fullfile(phaseAmp_sessionDir, fig_saveName{iVar, iPlotType}{iFreq});
                    end

                end    % for iType...
            end
            if exist(fig_saveName{1,1}{1}, 'file'); continue; end
            ...
            phaseAmp_name_mrl = [sessionChannels{1}.name '_' trialType '_phase_amp_mrl.mat'];
...
%             high_freqs = centerFreqs_1Hz(high_freq_idx);
            ...
            low_freqs = phaseAmp_metadata.low_freqs;
            high_freqs = phaseAmp_metadata.high_freqs;
            ...
            for iRegion = 1 : numRegions
                ...
                mrl_by_region{iRegion} = zeros(numCh, numEventTypes, length(low_freqs), length(high_freqs), length(t));
                mrl_z_by_region{iRegion} = zeros(numCh, numEventTypes, length(low_freqs), length(high_freqs), length(t));
