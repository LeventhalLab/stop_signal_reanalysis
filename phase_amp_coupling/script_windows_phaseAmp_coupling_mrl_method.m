% script_windows_phaseAmp_coupling
...
%             numSteps = length(winStartTimes);    %floor((range(eventtWin) - analysisWin) / stepSize) + 1;
            ...
            for iCh = 1 : numCh
                ...
                phaseAmp_metadata.chList{iCh} = ch.name;
                phaseAmp_metadata.regionList{iCh} = ch.location.name;

                [mrv_norm, surrogate, low_freqs, high_freqs] = phase_amp_Canolty_MI_20140224(ch, ...
                ...
                re_mrv_norm = real(mrv_norm); im_mrv_norm = imag(mrv_norm);
                save(phaseAmp_name_mrl, 're_mrv_norm', 'im_mrv_norm', 'surrogate', 'phaseAmp_metadata');
