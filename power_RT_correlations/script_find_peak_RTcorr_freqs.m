...
EOI_list = {'tone','nosecenterout'};    % events of interest

targetTimes = [0.25,0.5,0.75];
numTargetTimes = length(targetTimes);
...
figProps.m = channels_per_page; figProps.n = length(EOI_list) * numTargetTimes;   % number of rows and columns, respectively
...
PDFname = fullfile(RTcorr_plots_directory, 'powerRTcorr_across_subjects_timeSlice.pdf');
...
for iROI = 1 : length(ROI_list)
    ...
    for iEOI = 1 : length(EOI_list)
        ...
        for iTargetTime = 1 : numTargetTimes
            colNum = (iEOI - 1) * numTargetTimes + iTargetTime;
            axes(h_axes(iROI, colNum));
            
            targetIdx = find(abs(t - targetTimes(iTargetTime)) == min(abs(t - targetTimes(iTargetTime))));
        
            toPlot = squeeze(RTcorr_across_subjects(iROI, iEOI, :, targetIdx));
            plot(f, toPlot);

            set(gca,'ylim',yLim);
            if iROI == 1
                title([eventList{EOI_idx(iEOI)} ', ' num2str(targetTimes(iTargetTime)) 's']);
            end
