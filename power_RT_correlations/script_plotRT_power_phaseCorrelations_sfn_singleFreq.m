...
numFreqs_power = size(powerRTcorr, 2);
numFreqs_phase = size(circRTcorr, 2);
...
numSubs = 4;
powerRTcorrSubjects = zeros(numSubs, length(ROI_list), length(eventList), numFreqs_power, numSamps);
phaseRTcorrSubjects = zeros(numSubs, length(ROI_list), length(eventList), numFreqs_phase, numSamps);
        ...
        for iEOI = 1 : length(EOI_list)
            if ~isnan(ROI_idx(i_chDB,iROI))
                ...
                phaseRTcorrSubjects(i_chDB, iROI, iEOI, :, :) = squeeze(phaseRTcorrSubjects(i_chDB, iROI, iEOI, :, :)) + ...
                                                                   squeeze(mean_phaseRTcorr(ROI_idx(i_chDB,iROI), EOI_idx(iEOI), :, :));
            end
        end
    end
...
powerRTcorr_across_subjects = zeros(length(ROI_list), length(eventList), numFreqs_power, numSamps);
phaseRTcorr_across_subjects = zeros(length(ROI_list), length(eventList), numFreqs_phase, numSamps);
regionString = '';
numSubjectsString = '';
for iROI = 1 : length(ROI_list)
    ...
    for iEOI = 1 : length(EOI_list)
        ...
        plot(t, mean_power_toPlot,'k','linewidth',2);
        hold on
        plot(t, mean_phase_toPlot,'r','linewidth',2);
        ...
        for iSubject = 1 : numSubs
            if ~isnan(ROI_idx(iSubject, iROI))
                ...
                plot(t, power_toPlot,'color',[0.5 0.5 0.5],'linestyle','--');
                
                phase_toPlot = squeeze(phaseRTcorrSubjects(iSubject, iROI, iEOI, phase_freqIdx, :));
                plot(t, phase_toPlot,'color',[0.5 0 0],'linestyle','--');
            end
        end     
