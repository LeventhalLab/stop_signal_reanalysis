% script_plot_powerRTcorr_singleFreq
...
for i_chDB = 1 : length(chDB_list)
    ...
    mean_sessionRTcorr = zeros(numSessions, numRegions, numEvents, numSamps);
    ...
    for iSession = 1 : numSessions
        ...
        PDFname = fullfile(subject_RTcorr_plots_directory, [sessionList{iSession} '_power_RTcorr_' freqStr '.pdf']);
        ...
        for iCh = 1 : numCh
            ...
            freqIdx = find(abs(powerRTcorr_metadata.freqs - targetFreq) == min(abs(powerRTcorr_metadata.freqs - targetFreq)));
            
            mean_sessionRTcorr(iSession, regionIdx, :, :) = squeeze(mean_sessionRTcorr(iSession, regionIdx, :, :)) + squeeze(powerRTcorr(1:numEvents, freqIdx, :));
            ...
            for iEventType = 1 : numEvents
                ...
                toPlot = squeeze(powerRTcorr(iEventType, freqIdx, :));
                plot(t, toPlot);
                set(gca,'ylim',[-1 1]);
    ...
    mean_RTcorr = zeros(numRegions, numEvents, numSamps);
