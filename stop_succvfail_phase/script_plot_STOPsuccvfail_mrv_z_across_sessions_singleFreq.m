...
%     end
    ...
    zphase_acrossSessions_fig_saveName = [implantID '_zphase_stopSuccvFail_' freqStr '.pdf'];
    ...
    if ~exist(vecDiff_z_acrossSessions_mat_saveName,'file'); continue; end
...
%         mrvDiff = squeeze(region_mrv(iRegion,1,:,:,:) - region_mrv(iRegion,2,:,:,:));
        ...
        for iEventType = 1 : numEventTypes
            ...
            z_toPlot = squeeze(region_z(iRegion,iEventType, freqIdx, :));
            plot(t,z_toPlot);
            hold on
            set(gca,'ylim', yLim);
