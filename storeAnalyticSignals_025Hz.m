...
freq_band_width = 1;    % width of filtered bands
freq_interval_width = 0.25;    % spacing between filtered bands
center_freq     = ((1 + (freq_band_width/2)) : freq_interval_width : 10)';
numBands        = length(center_freq);
band_edge_width = 0.5;
% freq_bands = 1 : 2 : 101;    % borders of frequency bands at which to calculate the hilbert transforms

hilbert_directory = sprintf('/Volumes/PublicLeventhal1/dan/stop-signal reanalysis/Hilbert transformed LFP 025 Hz bins');
...
for i_chDB = 7:7%length(chDB_list) : length(chDB_list)
    
    if i_chDB > 2 && i_chDB < 5; continue; end
    ...
    for iSession = 1 : numSessions        
        ...
        if length(lfp_fileinfo) == 2
            lfp_fileinfo = lfp_fileinfo(1);
        end
        ...
        for iCh = metadata.numWrittenChannels + 1 : numCh
            ...
            for iFreq = metadata.numWrittenFreqs + 1 : numBands
                ...
                disp(['Filtering ' sessionList{iSession} ', ' ...
                      header.channel(lfp_idx).name ...
