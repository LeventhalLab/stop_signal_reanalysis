...
lfp_root          = '\\172.20.138.142\PublicLeventhal1\dan\stop-signal reanalysis\stop-signal LFPs';

[chDB_list, chDB_fnames] = get_chStructs_for_analysis_Nico;
...
% hilbert_directory = sprintf('\\141.214.45.212\PublicLeventhal1\dan\stop-signal reanalysis\Hilbert transformed LFP %d Hz bins', freq_band_width);
hilbert_directory = '\\172.20.138.142\PublicLeventhal1\dan\stop-signal reanalysis\Hilbert transformed LFP 1 Hz bins';
...
for i_chDB = 6 : 6%length(chDB_list)
    ...
    implantID = implantID_from_ratID_Nico(chDB_list{i_chDB}(1:5));
...
%         cp.locationSubClass = {'EMG', 'EEGLAM'};
        ...
        sessionDateVec = datevec(sessionDate, 'yyyy-mm-dd');
        sessionDate = datestr(sessionDateVec, 'yyyymmdd');
        ...
        for iCh = metadata.numWrittenChannels + 1 : numCh
            ...
            for iFreq = metadata.numWrittenFreqs + 1 : numBands
                ...
                disp([sessionChannels{iCh}.name ', ' ...
                      'Filtering ' header.channel(lfp_idx).name ...
