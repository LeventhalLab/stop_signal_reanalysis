...
hilbert_directory = sprintf('/Volumes/%s/dan/stop-signal reanalysis/Hilbert transformed LFP %d Hz bins', publicDir, freq_band_width);
...
for i_chDB = length(chDB_list) : length(chDB_list)
...
%         end
        ...
        for iCh = metadata.numWrittenChannels + 1 : numCh
            ...
            for iFreq = metadata.numWrittenFreqs + 1 : numBands
                ...
                disp(sprintf('Filtering %s, %s, %s, %d of %d channels, wire %d, centered at %f Hz', ...
                             header.subject, ...
                             header.date, ...
